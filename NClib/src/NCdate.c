...
NCstate NCtimePrint (NCtimeStep timeStep, utUnit *tUnit, double t, char *timeStr)
{
	...
	switch (timeStep)
	{
		...
		case NCtimeMonth:  sprintf (timeStr,"%04d-%02d",                     year, month); break;
		case NCtimeDay:    sprintf (timeStr,"%04d-%02d-%02d",                year, month, day); break;
		case NCtimeHour:   sprintf (timeStr,"%04d-%02d-%02d %02d",           year, month, day, hour); break;
		case NCtimeMinute: sprintf (timeStr,"%04d-%02d-%02d %02d:%02d",      year, month, day, hour, minute); break;
		case NCtimeSecond: sprintf (timeStr,"%04d-%02d-%02d %02d:%02d %.1f", year, month, day, hour, minute,second); break;
	}
	return (NCsucceeded);
}
...
const char *NCtimeStepString (NCtimeStep timeStep)
{
	switch (timeStep)
	{
		case NCtimeYear:   return ("year"); 
		case NCtimeMonth:  return ("month");
